...
/*
 *      RENDERABLE VELOCITY ENTITY CHUNK
 */
    RenderableVelEC::RenderableVelEC() : EntityChunk(s_type)
    {
        Memory &m = GM.memory();
        comps_.emplace_back(m.generalAllocT<LTransform>(kEntitiesPerChunk));
        comps_.emplace_back(m.generalAllocT<Drawable>(kEntitiesPerChunk));
        comps_.emplace_back(m.generalAllocT<Velocity>(kEntitiesPerChunk));
        comps_.emplace_back(m.generalAllocT<GTransform>(kEntitiesPerChunk));
    }

    RenderableVelEC::~RenderableVelEC()
    {
        Memory &m = GM.memory();
        m.generalFree(comps_[0]);
        m.generalFree(comps_[1]);
    ...
    void RenderableVelEC::relocateLast(EntityChunk *a, uint32_t i)
    {
        LEEP_ASSERT(a && i < kEntitiesPerChunk, "Wrong parameters");
        RenderableVelEC *chunk = static_cast<RenderableVelEC*>(a);
        // Update pointers in the scene graph
        GM.scene().scene_graph_.changeTransform(
            static_cast<LTransform*>(&(comps_[0][last_ - 1])),
            static_cast<LTransform*>(&(chunk->comps_[0][i])),
            static_cast<GTransform*>(&(chunk->comps_[3][i])));
        // Do the relocation
        static_cast<LTransform*>(chunk->comps_[0])[i] = static_cast<LTransform*>(comps_[0])[last_ - 1];
        static_cast<Drawable*>(chunk->comps_[1])[i] = static_cast<Drawable*>(comps_[1])[last_ - 1];
        static_cast<Velocity*>(chunk->comps_[2])[i] = static_cast<Velocity*>(comps_[2])[last_ - 1];
        static_cast<GTransform*>(chunk->comps_[3])[i] = static_cast<GTransform*>(comps_[3])[last_ - 1];
#ifdef LEEP_DEBUG
        static_cast<LTransform*>(comps_[0])[last_ - 1] = LTransform();
        static_cast<Drawable*>(comps_[1])[last_ - 1] = Drawable();
        static_cast<Velocity*>(comps_[2])[last_ - 1] = Velocity();
        static_cast<GTransform*>(comps_[3])[last_ - 1] = GTransform();
#endif
