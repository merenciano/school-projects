...
void TESTBASE_printFunctionResult(void *address, u8 *msg, s16 error_type) {
  ...
  case kErrorCode_NONE:
    strcpy((char *)error_msg, "Error NONE (not assigned)");
  ...
  case kErrorCode_Memory_Allocation:
    strcpy((char *)error_msg, "Memory allocation failed");
		printf("FAIL with error %d (%s)", error_type, error_msg);
    break;
    
  case kErrorCode_NULL:
    strcpy((char *)error_msg, "NULL not allowed");
		printf("FAIL with error %d (%s)", error_type, error_msg);
    break;

  case kErrorCode_NULL_Data:
    strcpy((char *)error_msg, "NULL data");
		printf("FAIL with error %d (%s)", error_type, error_msg);
    break;
    
  case kErrorCode_Param:
    strcpy((char *)error_msg, "Parameter not valid");
		printf("FAIL with error %d (%s)", error_type, error_msg);
    break;
  ...
  case kErrorCode_Param_Zero_Size:
    strcpy((char *)error_msg, "Zero size (parameter)");
		printf("FAIL with error %d (%s)", error_type, error_msg);
    break;
    
  case kErrorCode_NULL_Storage:
    strcpy((char *)error_msg, "NULL storage");
		printf("FAIL with error %d (%s)", error_type, error_msg);
    break;
    
  case kErrorCode_Full:
    strcpy((char *)error_msg, "Is full");
		printf("FAIL with error %d (%s)", error_type, error_msg);
    break;

  case kErrorCode_Empty:
    strcpy((char *)error_msg, "Is empty");
		printf("FAIL with error %d (%s)", error_type, error_msg);
    break;
    
  case kErrorCode_Not_Enough_Capacity:
    strcpy((char *)error_msg, "Not enough capacity");
		printf("FAIL with error %d (%s)", error_type, error_msg);
    break;
